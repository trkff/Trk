...
main.py                 <- Bot loop orchestration — event-driven via BinanceCandleManager WebSocket; `bot_loop()` chama `get_required_timeframes()` antes de criar o `BinanceCandleManager` (só subscreve TFs que estratégias habilitadas precisam — incluindo 15m/30m/1h que aparecem quando alguma instância tem `params['timeframe']` setado); heartbeat de 30s (verifica status, assets, TP/SL); `process_asset(asset, cfg, last_15m_ts, last_30m_ts, last_1h_ts, last_4h_ts, last_1d_ts)` é chamado pelo worker thread a cada 5m close — busca TODOS os TFs (5m, 15m, 30m, 1h, 4h, 1d) via `client.get_candles()` se o TF estiver em `candle_mgr.intervals`; `candle_mgr` é usado APENAS como trigger de candle close (evento 5m alinha boundaries de TFs maiores), não como fonte de dados; `new_5m=True` sempre; helper `_detect_new(tf_label, df, store)` compara último timestamp do df com store por asset — detecta `new_15m`, `new_30m`, `new_1h`, `new_4h`, `new_1d`; todos propagados para `evaluate_all` junto com `df_5m, df_15m, df_30m, df_1h, df_4h, df_1d`; `check_bb_mid_exit(asset, df_5m)` a cada 5m close; `candle_mgr` é global `BinanceCandleManager | None`; retry de candle stale: após buscar df_5m, se o candle mais recente tiver mais de 360s (6min), dorme 5s e retenta uma vez — protege co-trigger assets (WTI, HYPE) que podem ter lag no Lighter REST
bot/
...
- **Lighter**: candles via Lighter REST (`/api/v1/candles`) — o `LighterExchangeClient.get_candles()` chama diretamente o feed da Lighter (com fallback para Binance em caso de erro). Resoluções suportadas: `1m`, `5m`, `15m`, `30m`, `1h`, `4h`, `12h`, `1d`, `1w`. Máx 500 candles por chamada.
- **Hyperliquid**: candles via Binance Spot REST (`api.binance.com/api/v3/klines`) via `fetch_binance_candles()` em `bot/exchanges/base.py` (delegate do `HyperliquidExchangeClient.get_candles()`).
- Em ambos os casos, `process_asset` em `main.py` chama `client.get_candles()` — o `BinanceCandleManager` é usado apenas como trigger de candle close, não como fonte de dados.
