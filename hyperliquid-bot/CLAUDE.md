...
- **Lighter (REST fallback)**: `LighterExchangeClient.get_candles()` chama `/api/v1/candles` direto. Usado em (a) cold start / seed do buffer no `start()` e em `update_assets()`, (b) boundary fallback para canais sem update WS na janela, (c) read direto quando manager está offline. Em caso de erro ou market não encontrado, retorna `pd.DataFrame()` vazio — sem fallback Binance (removido na migração do WS Lighter). `main.py` tem retry stale de 360s que cobre falhas transitórias.
