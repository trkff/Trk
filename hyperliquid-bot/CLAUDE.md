...
- **PnL/stats per perfil:** `get_daily_pnl(profile_id=None)`, `get_total_pnl(profile_id=None)`, `get_trade_stats(profile_id=None)` agora aceitam `profile_id`. `None` = agregado em todos os perfis (mantém uso de scripts antigos). `/api/overview` e `background_pusher` passam `g.profile_id` / `pid` — cada perfil mostra seus próprios números no dropdown
